import streamlit as st
from ultralytics import YOLO
import os
from PIL import Image
import cv2
import numpy as np

# --- Konfigurasi Halaman Streamlit ---
st.set_page_config(
    page_title="Deteksi Slot Parkir",
    page_icon="🚗",
    layout="wide"
)

# --- Judul dan Deskripsi ---
st.title("🅿️ Deteksi Status Slot Parkir")
st.write(
    "Unggah gambar area parkir untuk mendeteksi slot yang kosong (empty) dan terisi (filled). "
    "Aplikasi ini menggunakan model YOLO yang telah di-fine-tune."
)

# --- Path ke Model ---
MODEL_PATH = 'best.pt'

# --- Fungsi untuk memuat model (dengan cache agar lebih cepat) ---
# Bagian ini sudah sempurna.
@st.cache_resource
def load_model(model_path):
    """Memuat model YOLO dari path yang diberikan."""
    try:
        model = YOLO(model_path)
        return model
    except Exception as e:
        st.error(f"Gagal memuat model: {e}")
        return None

# --- Fungsi utama untuk deteksi dan visualisasi ---
# Fungsi ini juga sudah sempurna. Kita akan memanggilnya nanti.
def detect_and_draw_on_image(
    input_image: Image.Image, 
    model: YOLO,
    conf_threshold: float = 0.25,
    iou_threshold: float = 0.45
) -> (np.ndarray, dict):
    
    # 1. Lakukan prediksi seperti biasa
    results = model.predict(
        source=input_image,
        conf=conf_threshold,
        iou=iou_threshold
    )
    
    # --- PERUBAHAN UTAMA DIMULAI DI SINI ---
    
    # 2. Siapkan gambar untuk digambar secara manual
    #    Konversi gambar PIL ke format NumPy array yang bisa digunakan OpenCV
    annotated_image_bgr = np.array(input_image.convert('RGB'))
    annotated_image_bgr = cv2.cvtColor(annotated_image_bgr, cv2.COLOR_RGB2BGR)

    # 3. Inisialisasi statistik
    stats = {'empty': 0, 'filled': 0}
    
    # 4. Loop melalui setiap kotak yang terdeteksi untuk menggambarnya secara manual
    for box in results[0].boxes:
        class_id = int(box.cls[0])
        
        # Tentukan warna berdasarkan class_id
        if class_id == 1: # 1: filled
            color = (0, 0, 255)  # Merah (dalam format BGR)
            stats['filled'] += 1
        else: # 0: empty
            color = (0, 255, 0)  # Hijau (dalam format BGR)
            stats['empty'] += 1
            
        # Ambil koordinat kotak
        coords = box.xyxy[0].cpu().numpy().astype(int)
        x1, y1, x2, y2 = coords
        
        # Gambar kotak di atas gambar dengan warna dan ketebalan yang kita tentukan
        cv2.rectangle(
            img=annotated_image_bgr, 
            pt1=(x1, y1), 
            pt2=(x2, y2), 
            color=color, 
            thickness=1 # Anda bisa ubah ketebalan garis di sini
        )

    # Hitung total
    stats['total'] = stats['empty'] + stats['filled']
    
    # --- Akhir dari Perubahan Utama ---
    
    # 5. Tambahkan legenda statistik kustom Anda (bagian ini tidak berubah)
    def draw_text_with_background(img, text, position, font, scale, text_color, bg_color, thickness=2, padding=10, alpha=0.6):
        overlay = img.copy()
        text_size, _ = cv2.getTextSize(text, font, scale, thickness)
        text_w, text_h = text_size
        x, y = position
        cv2.rectangle(overlay, (x - padding, y - text_h - padding), (x + text_w + padding, y + padding), bg_color, -1)
        cv2.addWeighted(overlay, alpha, img, 1 - alpha, 0, img)
        cv2.putText(img, text, (x, y), font, scale, text_color, thickness)

    font = cv2.FONT_HERSHEY_SIMPLEX
    # Gunakan warna yang sama dengan kotak untuk teks legenda
    draw_text_with_background(annotated_image_bgr, f"Filled Spots: {stats['filled']}", (30, 50), font, 1, (0, 0, 255), (255,255,255))
    draw_text_with_background(annotated_image_bgr, f"Empty Spots: {stats['empty']}", (30, 100), font, 1, (0, 255, 0), (255,255,255))
    draw_text_with_background(annotated_image_bgr, f"Total Spots: {stats['total']}", (30, 150), font, 1, (0,0,0), (255,255,255))

    # 6. Konversi final ke RGB untuk ditampilkan
    annotated_image_rgb = cv2.cvtColor(annotated_image_bgr, cv2.COLOR_BGR2RGB)

    return annotated_image_rgb, stats

# Muat model di awal
model = load_model(MODEL_PATH)

# UI untuk unggah file
uploaded_file = st.file_uploader(
    "Pilih sebuah gambar...", 
    type=["jpg", "jpeg", "png"]
)

if uploaded_file is not None:
    image = Image.open(uploaded_file)
    
    # DIUBAH: Kita tidak lagi menggunakan 2 kolom, tapi 5 kolom untuk kontrol penuh
    # Rasio: [spacer, gambar1, spacer, gambar2, spacer]
    col1, col2, col3, col4, col5 = st.columns([1, 3.5, 1, 3.5, 1])
    col123 = st.columns(2)
    col11,col12 = st.columns([1, 1])

    # --- GAMBAR ASLI (DI KOLOM KEDUA) ---
    with col2:
        # DIUBAH: Buat 2 sub-kolom DI DALAM col2 untuk header dan tombol
        # Kita beri rasio [3, 1] agar header mendapat lebih banyak ruang
        header_col, button_col = st.columns([3, 1])

        with header_col:
            st.subheader("Gambar Asli")

        with button_col:
            # Kita beri spasi kosong agar posisi tombol sedikit turun
            # dan lebih sejajar dengan teks subheader
            st.write("") 
            start_detection = st.button("Deteksi!", use_container_width=True)

        # Gambar ditempatkan SETELAH baris header/tombol, tapi masih di dalam col2
        st.image(image, caption="Gambar yang Anda unggah.", use_column_width=True)

    # Tombol deteksi kita letakkan di kolom tengah (kolom 3) agar terlihat rapi
    # with col3:
    #     # Beri sedikit ruang dari atas agar tombol tidak terlalu menempel
    #     st.subheader('===>')

    # Jika tombol ditekan, kita proses dan tampilkan hasilnya di kolom keempat
    if start_detection:
        if model is not None:
            # Gunakan spinner di kolom output agar tidak mengganggu tata letak
            with col4:
                with st.spinner('Sedang memproses...'):
                    # Panggil fungsi utama Anda
                    final_image, stats = detect_and_draw_on_image(
                        input_image=image, 
                        model=model,
                        conf_threshold=0.25
                    )

                    # --- GAMBAR HASIL (DI KOLOM KEEMPAT) ---
                    st.subheader("Hasil Deteksi")
                    # Gunakan use_column_width agar gambar mengisi penuh 30% kolomnya
                    st.image(final_image, caption="Gambar dengan deteksi.", use_column_width=True)
                    
                    # Tampilkan metrik di bawah gambar hasil
                    st.divider()
                    m_col1, m_col2, m_col3 = st.columns(3)
                    with m_col1:
                        st.metric(label="Total Slot", value=stats['total'])
                    with m_col2:
                        st.metric(label="Terisi", value=stats['filled'])
                    with m_col3:
                        st.metric(label="Kosong", value=stats['empty'])
        else:
            st.error("Model tidak berhasil dimuat.")
else:
    st.info("Silakan unggah sebuah gambar untuk memulai.")